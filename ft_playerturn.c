...
/*   Updated: 2016/02/27 13:38:19 by gvilmont         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_askcolumn(int fd, int l, int h)
{
	int		ret;
	char	*line;
	int		pcolumn;
...
int		ft_columnlen(char **tab, int pcolumn)
{
	...
	while (a < g_h)
	{
		if (tab[a][pcolumn] != '.' && a != 0)
			return (a);
		if (tab[a][pcolumn] != '.' && a == 0)
		{
			ft_putstr("ERROR, you can not place it here\n");
			ft_filltab(tab, g_l, g_h);
		}
		a++;
	}
	return (a);
...
char	**ft_filltab(char **tab, int l, int h)
{
	...
	pcolumn = ft_askcolumn(0, g_l, g_h) - 1;
